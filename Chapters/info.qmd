...
::: proof
 A one-to-one mapping $\srcblk \mapsto \enc^{k}$ is invertible. If we enumerate all elements of $\srcblk$ in binary, we will need $k$ bits. Thus, with absolute certainty: $$\begin{aligned}
...
::: proof
 From the theorem definition, $\src_i$ are independent. Then from the Product Rule (eq. [\[eq:Product_Rule\]][22]): $$\begin{aligned}
...
::: proof
 We refer to [@cover:2006 th.2.8.1] for proof. ◻
:::
...
::: proof
 $\rvZ=g(\rvY) \therefore I[\rvX;g(\rvY)]=I[\rvX;\rvZ]$. By the [DPI]{acronym-label="DPI" acronym-form="singular+full"} property: $$\begin{aligned}
        I[\rvX;\rvY] &\geqslant I[\rvX;\rvZ]\\
