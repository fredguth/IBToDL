...
::: proof
Notice that the output weight $\rvW$ of the training process can be seen as a random variable (that depends on the stochasticity of the initialisation, training steps, and the data); i.e. $\rvW$ is a representation of the dataset $\rvD$ and we can talk about $\IWD$.
